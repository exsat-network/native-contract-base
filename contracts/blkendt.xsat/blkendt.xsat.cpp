...
void block_endorse::config(const uint64_t limit_endorse_height, const uint16_t limit_num_endorsed_blocks,
    ...
    check(min_xsat_qualification.symbol == XSAT_SYMBOL, "blkendt.xsat::config: min_xsat_qualification symbol must be XSAT");
...
void block_endorse::endorse(const name& validator, const uint64_t height, const checksum256& hash) {
    ...
    check(config.limit_num_endorsed_blocks == 0 || chain_state.parsed_height + config.limit_num_endorsed_blocks >= height,
    ...
    bool xsat_validator = validator_itr->role.value() == 1;
    ...
    if (endorsement_itr == endorsement_idx.end()) {
        ...
        // check validator role is xsat validator

        // Obtain qualified validators based on the pledge amount.
        // If the block height of the activated xsat pledge amount is reached, directly switch to xsat pledge, otherwise use the
        // btc pledge amount.
        ...
        check(std::find_if(endorsement_itr->provider_validators.begin(), endorsement_itr->provider_validators.end(),
                           [&](const provider_validator_info& a) { return a.account == validator; })
...
block_endorse::get_valid_validator_by_btc_stake(const uint64_t min_btc_qualification) {
    endorse_manage::validator_table _validator
        = endorse_manage::validator_table(ENDORSER_MANAGE_CONTRACT, ENDORSER_MANAGE_CONTRACT.value);
...
std::vector<block_endorse::requested_validator_info>
block_endorse::get_valid_validator_by_xsat_stake(const uint64_t min_xsat_qualification) {
    endorse_manage::validator_table _validator
        = endorse_manage::validator_table(ENDORSER_MANAGE_CONTRACT, ENDORSER_MANAGE_CONTRACT.value);
    ...
    while (itr != idx.end()) {
        ...
        result.emplace_back(
            requested_validator_info{.account = itr->owner, .staking = static_cast<uint64_t>(itr->xsat_quantity.amount)});
        itr++;
    }
    return result;
}